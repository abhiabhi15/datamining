...
def is_same_station_in_time(node1, node2):
    return node1.split("_")[0] == node2.split("_")[0] and node1.split("_")[1] != node2.split("_")[1]
...
def connect_time_layer_node_edges(G):
    ...
    for unode in u_list:
        for other_unode in u_list:
            if is_same_station_in_time(unode, other_unode):
                 t = unode, other_unode, 1
                 edges.append(t)

    for vnode in u_list:
        for other_vnode in u_list:
            if is_same_station_in_time(vnode, other_vnode):
...
def create_edge_feature_DF(G):
    ...
    for k,v in G.edges_iter():
        for feature in feature_list:
            ...
            fdiff.append(abs(G.node[k][FEATURES][feature] - G.node[v][FEATURES][feature]) + 0.00001)
...
def init_edge_weights(G):
    ...
    for k, v in G.edges_iter():
        ...
        summation = 0
        for feature in feature_list:
            fdiff = node_features.loc[node_features["fname"] == feature].iloc[0]["feature_diff"]
...
def preprocess_DF(df):
    ...
    df[feature_list] = df[feature_list].apply(lambda x: (x - np.mean(x)) / (np.max(x) - np.min(x)))
    return df
...
def write_predicted_results(predicted_list):
    ...
    predicted_df.insert(0, "id", u_list)
